...
class WP_Webhooks_Pro_Authentication {
	...
	public function apply_authentication_template_data( $data, $response, $webhook, $args, $authentication_data ){
	 ...
	 public function apply_authentication_template_header( $http_args, $args, $url, $webhook, $authentication_data ){
		...
		switch( $auth_type ){
			...
			break;
			case 'digest_auth':
				$http_args = $this->validate_http_digest_header( $http_args, $auth_data, $webhook );
			break;
		}
    ...
    public function validate_http_digest_header( $http_args, $auth_data, $webhook ){
	...
	public function action_validate_api_key( $template_data ){
		$return = array(
			'success' => false,
			'msg' => WPWHPRO()->helpers->translate( 'No response message was added.', 'wpwhpro-page-authentication' ),
		);
