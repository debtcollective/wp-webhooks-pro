    ...
    <form id="wpwh-main-settings-form" method="post" action="">
		...
		<div class="wpwh-title-area mb-4">
			...
			<?php if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_settings' ) ) ) : ?>
				<p><?php echo WPWHPRO()->helpers->translate( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_settings' ), 'admin-settings-license' ); ?></p>
			<?php else : ?>
				<p><?php echo sprintf( WPWHPRO()->helpers->translate( 'Down below you can customize the global settings for %s. Please make sure to read the settings descriptions carefully before saving the settings.', 'wpwhpro-page-settings' ), WPWHPRO()->settings->get_page_title() ); ?></p>
			<?php endif; ?>
		</div>
		...
		<div class="wpwh-title-area mb-4 mt-4">
			...
			<p class="wpwh-text-small"><?php echo sprintf( WPWHPRO()->helpers->translate( 'The settings down below are very powerful and have a huge impact to the functionality of the plugin. Please use them with caution.', 'wpwhpro-page-settings' ), WPWHPRO()->settings->get_page_title() ); ?></p>
		</div>
