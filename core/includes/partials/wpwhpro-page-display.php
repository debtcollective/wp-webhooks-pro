...
if( isset( $_GET['wpwhprovrs'] ) && $_GET['wpwhprovrs'] != 'home' ){
	...
	$active_val      = 'home';
    $remote_content      = WPWHPRO()->api->get_news_feed( WPWHPRO_NEWS_FEED_ID );

    if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_home' ) ) ){
        $remote_content = WPWHPRO()->helpers->validate_local_tags( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_home' ) );
    }
	...
	$current_content = ! empty( $current_content ) ? $current_content : WPWHPRO()->helpers->translate( 'Welcome to WP Webhook Pro! Currently we are not able to show you the newest informations.', 'admin-backend' ) ;
}
	...
	foreach( $menu_endpoints as $hook_name => $data ){
		...
		if( current_user_can( apply_filters( 'wpwhpro/admin/settings/menu/page_capability', WPWHPRO()->settings->get_admin_cap( 'wpwhpro-page-settings' ), $active_val ) ) ){
			...
			if( $active_val == $hook_name || $has_dropdown && isset( $data['items'][ $active_val ] ) ){
                $heading .= '<li class="active' . ( $has_dropdown ? ' has-dropdown' : '' ) . '"><a class="ironikus-setting-single-tab active ' . $hook_name . '" ' . $html_title . ' href="?page=' . $this->page_name . '&wpwhprovrs=' . $hook_name . '">' . $title . ( $has_dropdown ? ' ' . $dd_svg : '' ) . '</a>';
			} else {
				$heading .= '<li' . ( $has_dropdown ? ' class="has-dropdown"' : '' ) . '><a class="ironikus-setting-single-tab ' . $hook_name . '" ' . $html_title . ' href="?page=' . $this->page_name . '&wpwhprovrs=' . $hook_name . '">' . $title . ( $has_dropdown ? ' ' . $dd_svg : '' ) . '</a>';
            }
            ...
            if( $has_dropdown ){
                ...
                foreach( $data['items'] as $sub_menu_name => $sub_menu_title ){
                    ...
                    if( $active_val == $sub_menu_name ){
                        $heading .= '<li class="active"><a class="ironikus-setting-single-tab active ' . $sub_menu_name . '" href="?page=' . $this->page_name . '&wpwhprovrs=' . $sub_menu_name . '">' . $sub_menu_title . '</a></li>';
                    } else {
...
<div class="wpwh">
    <div class="wpwh-header">
        <div class="wpwh-container">
            <?php if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_name' ) ) ) : ?>
                <span class="wpwh-header__logo-text"><?php echo WPWHPRO()->helpers->translate( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_name' ), 'admin-settings-license' ); ?></span>
            <?php else : ?>
