...
if ( ! class_exists( 'WP_Webhooks_Integrations_groundhogg_Actions_ghogg_user_tags_remove' ) ) :
	...
	public function get_details(){
		...
		$translation_ident = "action-ghogg_user_tags_remove-content";
			...
			$parameter = array(
				...
				'tags'	=> array( 'required' => true, 'short_description' => WPWHPRO()->helpers->translate( 'Add the tags you want to remove from the contact. This argument accepts a comma-separated string, as well as a JSON construct.', $translation_ident ) ),
...
<pre>{
  ...
  44
		...
		$parameter['tags']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>ghogg_user_tags_remove</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
			'webhook_name' => 'Remove user tags',
			'webhook_slug' => 'ghogg_user_tags_remove',
			'steps' => array(
				WPWHPRO()->helpers->translate( 'It is also required to set the <strong>contact_value</strong> argument. Please set it to the contact id or contact/user email of the contact you want to remove the tags from.', $translation_ident ),
		...
		return array(
			...
			'name'			   => WPWHPRO()->helpers->translate( 'Remove user tags', $translation_ident ),
			'sentence'			   => WPWHPRO()->helpers->translate( 'remove one or multiple tags from a user', $translation_ident ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => WPWHPRO()->helpers->translate( 'Remove one or multiple tags from a user within Groundhogg.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $contact_value ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the contact_value argument.", 'action-ghogg_user_tags_remove-error' );
				return $return_args;
			}

			if( empty( $tags ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the tags argument.", 'action-ghogg_user_tags_remove-error' );
				return $return_args;
			}
			...
			if( ! $contact->exists() ) {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The contact you try to update does not exist.", 'action-ghogg_user_tags_remove-error' );
				return $return_args;
			}
			...
			$return_args['msg'] = WPWHPRO()->helpers->translate( "Tags have been removed from the respective contact.", 'action-ghogg_user_tags_remove-success' );
