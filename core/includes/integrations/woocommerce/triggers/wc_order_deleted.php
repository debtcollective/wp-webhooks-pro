...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Triggers_wc_order_deleted' ) ) :

 /**
  * Load the wc_order_deleted trigger
  *
  * @since 4.3.2
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_woocommerce_Triggers_wc_order_deleted {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				'type' => 'action',
				'hook' => 'wp_trash_post',
				'callback' => array( $this, 'wc_order_deleted_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => false,
			),
	...
	public function get_details(){
		...
		$translation_ident = "trigger-wc_order_deleted-description";
		...
		return array(
			...
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as an order was deleted within Woocommerce.', $translation_ident ),
	...
	/**
	 * Triggers once a order was deleted
	 *
	 * @param mixed $arg
	 */
	public function wc_order_deleted_callback( $arg ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wc_order_deleted' );
		$payload = array();
		$payload_track = array();
		...
		do_action( 'wpwhpro/webhooks/trigger_wc_order_deleted', $payload, $response_data_array, $payload_track );
	}
