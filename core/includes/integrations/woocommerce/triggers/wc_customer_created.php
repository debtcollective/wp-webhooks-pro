...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Triggers_wc_customer_created' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_woocommerce_Triggers_wc_customer_created {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				'type' => 'action',
				'hook' => 'user_register',
				'callback' => array( $this, 'wc_customer_created_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => true,
			),
	...
	/**
	 * Triggers once a customer was created
	 *
	 * @param mixed $arg
	 */
	public function wc_customer_created_callback( $arg ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wc_customer_created' );
		$payload = array();
		$payload_track = array();
		...
		do_action( 'wpwhpro/webhooks/trigger_wc_customer_created', $payload, $response_data_array, $payload_track );
	}
