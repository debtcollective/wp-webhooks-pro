...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Triggers_wc_order_updated' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_woocommerce_Triggers_wc_order_updated {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'wc_order_updated_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => true,
			),
	...
	/**
	 * Triggers once a order was updated
	 *
	 * @param mixed $arg
	 */
	public function wc_order_updated_callback( $arg ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wc_order_updated' );
		$payload = array();
		$payload_track = array();
		...
		do_action( 'wpwhpro/webhooks/trigger_wc_order_updated', $payload, $response_data_array, $payload_track );
	}
