...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Triggers_wc_customer_deleted' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_woocommerce_Triggers_wc_customer_deleted {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				'type' => 'action',
				'hook' => 'delete_user',
				'callback' => array( $this, 'wc_customer_deleted_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => false,
			),
	...
	/**
	 * Triggers once a customer was deleted
	 *
	 * @param mixed $arg
	 */
	public function wc_customer_deleted_callback( $arg ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wc_customer_deleted' );
		$payload = array();
		$payload_track = array();
		...
		do_action( 'wpwhpro/webhooks/trigger_wc_customer_deleted', $payload, $response_data_array, $payload_track );
	}
