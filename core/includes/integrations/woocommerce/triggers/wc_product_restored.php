...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Triggers_wc_product_restored' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_woocommerce_Triggers_wc_product_restored {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				'type' => 'action',
				'hook' => 'untrashed_post',
				'callback' => array( $this, 'wc_product_restored_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => true,
			),
	...
	/**
	 * Triggers once a product was restored
	 *
	 * @param mixed $arg
	 */
	public function wc_product_restored_callback( $arg ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wc_product_restored' );
		$payload = array();
		$payload_track = array();
		...
		do_action( 'wpwhpro/webhooks/trigger_wc_product_restored', $payload, $response_data_array, $payload_track );
	}
