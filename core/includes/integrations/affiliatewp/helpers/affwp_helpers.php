...
if ( ! class_exists( 'WP_Webhooks_Integrations_affiliatewp_Helpers_affwp_helpers' ) ) :
	 ...
	 */
	class WP_Webhooks_Integrations_affiliatewp_Helpers_affwp_helpers {

		public function get_affiliate_statuses( $entries, $query_args, $args ){
			...
			$affiliate_items = affwp_get_affiliate_statuses();

			foreach( $affiliate_items as $name => $title ){
		...
		public function get_referral_statuses( $entries, $query_args, $args ){
			...
			$referral_items = affwp_get_referral_statuses();

			foreach( $referral_items as $name => $title ){
		...
		public function get_referral_types( $entries, $query_args, $args ){
			...
			foreach ( affiliate_wp()->referrals->types_registry->get_types() as $type_slug => $type ) {
				$referral_items[ $type_slug ] = ( isset( $type['label'] ) && ! empty( $type['label'] ) ) ? sanitize_text_field( $type['label'] ) : $type_slug;
			}
		...
		public function get_affiliate_integrations( $entries, $query_args, $args ){
			...
			foreach( $integrations_enabled as $slug => $data ){
				...
				$integration_items[ $slug ] = $integration_name;
			}
