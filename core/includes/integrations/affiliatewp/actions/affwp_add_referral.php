...
<ol>
	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			$return_args = array(
				'success' => false,
				'msg' => WPWHPRO()->helpers->translate( "No referral was created.", 'action-affwp_add_referral-error' ),
				'data' => array(),
			);
			...
			if( empty( $user_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "We have trouble to find a user for your given user_id.", 'action-affwp_add_referral-error' );
				return $return_args;
			}
	
			if( empty( $affiliate_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "We have trouble to find an affiliate for your given affiliate_id.", 'action-affwp_add_referral-error' );
				return $return_args;
			}

			$user = get_user_by( 'id', $user_id );
			if( empty( $user ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "We have trouble to get the found user.", 'action-affwp_add_referral-error' );
				return $return_args;
			}
			...
			if( ! empty( $referral_id ) ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Referral was successfully created.", 'action-affwp_add_referral-success' );
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while creating your referral.", 'action-affwp_add_referral-success' );
			}
