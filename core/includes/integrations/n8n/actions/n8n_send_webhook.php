<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_n8n_Actions_n8n_send_webhook' ) ) :

	/**
	 * Load the n8n_send_webhook action
	 *
	 * @since 5.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_n8n_Actions_n8n_send_webhook {

		public function get_details(){

			$translation_ident = "action-n8n_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'n8n "Webhook" URL', $translation_ident ), 
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhooks" app from within n8n.', $translation_ident ), 
				),
				'method'	=> array( 
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>n8n_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$returns_code = array (
				...
				array (
				  ...
				  array (
					...
					'date' => 'Thu, 24 Mar 2022 18:51:20 GMT',
					...
					'etag' => 'x/"xx-xxxxxxxxxxxxxxxxxxxxxxxx"',
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to n8n webhook app',
				'webhook_slug' => 'n8n_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhooks" app of n8n.', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'n8n_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to n8n webhook app', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the n8n webhook app', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to the "Webhooks" app of n8n from your WordPress website.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-n8n_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-n8n_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-n8n_send_webhook-success' );
			}
