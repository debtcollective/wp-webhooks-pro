...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_memberships_Triggers_wcm_membership_cancelled' ) ) :

 /**
  * Load the wcm_membership_cancelled trigger
  *
  * @since 4.3.7
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_woocommerce_memberships_Triggers_wcm_membership_cancelled {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'wcm_membership_cancelled_callback' ),
				'priority' => 20,
				'arguments' => 3,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "trigger-wcm_membership_cancelled-description";

		$parameter = array(
			'user_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The ID of the user the membership was cancelled for.', $translation_ident ) ),
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			...
			'webhook_slug' => 'wcm_membership_cancelled',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'wc_memberships_user_membership_status_changed',
		...
		return array(
			'trigger'		   => 'wcm_membership_cancelled',
			'name'			  => WPWHPRO()->helpers->translate( 'Membership cancelled', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a membership was cancelled', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a membership was cancelled within WooCommerce Memberships.', $translation_ident ),
	...
	public function wcm_membership_cancelled_callback( $user_membership, $old_status, $new_status ){	

		if( $old_status === $new_status || $new_status !== 'cancelled' ){
			return;
		}

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wcm_membership_cancelled' );
		$payload = array(
		...
		do_action( 'wpwhpro/webhooks/trigger_wcm_membership_cancelled', $payload, $response_data_array );
	}
