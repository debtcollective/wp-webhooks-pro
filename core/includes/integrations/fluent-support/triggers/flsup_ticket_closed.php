  ...
  */
  class WP_Webhooks_Integrations_fluent_support_Triggers_flsup_ticket_closed {
	...
	public function get_details(){
		...
		$translation_ident = "action-flsup_ticket_closed-description";
		...
		$parameter = array(
			...
			'person' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Array) All details of the agent (or customer) that was closing this ticket.', $translation_ident ) ),
		);

		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Ticket closed',
			'webhook_slug' => 'flsup_ticket_closed',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'fluent_support/ticket_closed',
			...
			'tipps' => array(
				WPWHPRO()->helpers->translate( 'You can fire this trigger as well on specific person types only (E.g. when an Agent or Customer closes the ticket). To do that, select specific person_type from the webhook URL settings.', $translation_ident ),
			)
		) );
		...
		$settings = array(
			...
			'data' => array(
				'wpwhpro_fluent_support_trigger_on_person_type' => array(
					...
					'placeholder' => '',
					'required'	=> false,
					'description' => WPWHPRO()->helpers->translate( 'Trigger this webhook only when a specific type of person closes the ticket. You can also choose multiple ones. If none is selected, all are triggered.', $translation_ident )
				),
			)
		);
		...
		return array(
			...
			'name'			  => WPWHPRO()->helpers->translate( 'Ticket closed', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a ticket was closed', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a ticket was closed within Fluent Support.', $translation_ident ),
