				...
				foreach( $trigger_settings as $setting => $setting_data ){
					...
					if( isset( $setting_data['type'] ) ){
						...
						if( $setting_data['type'] === 'select' ){
							...
							if( $setting === 'wpwhpro_trigger_data_mapping' ){
								$choices = array_replace( $choices, WPWHPRO()->data_mapping->flatten_data_mapping_data( $data_mapping_templates ) );
							}
...
<ol>
	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			$return_args = array(
				'success' => false,
				'msg' => WPWHPRO()->helpers->translate( "No actions have been executed.", 'action-bulk_webhooks-success' ),
				'actions' => array(),
			);
			...
			if( empty( $actions ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The actions argument cannot be empty.", 'action-bulk_webhooks-success' );
				return $return_args;
			}
			...
			$return_args['msg'] = WPWHPRO()->helpers->translate( "All webhook calls have been executed.", 'action-bulk_webhooks-success' );
