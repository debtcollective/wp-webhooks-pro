...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_plugin_uninstall' ) ) :
	...
	class WP_Webhooks_Integrations_wordpress_Actions_plugin_uninstall {
		...
		public function get_details(){
			...
			$translation_ident = "action-plugin_uninstall-description";
...
<?php
			...
			ob_start();
?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>plugin_uninstall</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo WPWHPRO()->helpers->translate( "True if the plugin was successfully uninstalled, false if not.", $translation_ident ); ?>
	</li>
	<li>
...
<?php
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Uninstall a plugin',
				'webhook_slug' => 'plugin_uninstall',
				'steps' => array(
			...
			return array(
				...
				'name'			  => WPWHPRO()->helpers->translate( 'Uninstall plugin', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'uninstall a plugin', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to uninstall a plugin within your WordPress website.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $plugin_slug ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the plugin_slug to continue.", 'action-plugin_uninstall-failure' );
				return $return_args;
			}

			$check = $plugin_helpers->uninstall( $plugin_slug, array( 'force_delete'  =>  $force_delete ) );
			if( $check ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Plugin successfully uninstalled.", 'action-plugin_uninstall-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The plugin was not uninstalled.", 'action-plugin_uninstall-success' );
			}
