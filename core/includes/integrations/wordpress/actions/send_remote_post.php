...
<ol>
	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			$reject_unsafe_urls	 = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'reject_unsafe_urls' );
			$limit_response_size	 = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'limit_response_size' );
			$do_action		  = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'do_action' );

			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-send_remote_post-failure' );
				return $return_args;
			}
			...
			$check = wp_remote_post( $url, $arguments );

			$return_args['data'] = $check;	

			if( $check && ! is_wp_error( $check ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The request was sent successfully.", 'action-send_remote_post-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the request.", 'action-send_remote_post-success' );
			}
