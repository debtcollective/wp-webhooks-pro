...
<?php
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Activate a plugin',
				'webhook_slug' => 'plugin_activate',
				'steps' => array(
			...
			return array(
				...
				'name'			  => WPWHPRO()->helpers->translate( 'Activate plugin', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'activate a plugin', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $plugin_slug ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the plugin_slug to continue.", 'action-plugin_activate-failure' );
				return $return_args;
			}

			$check = $plugin_helpers->activate( $plugin_slug );
			if( $check ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Plugin successfully activated.", 'action-plugin_activate-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The plugin was not activated.", 'action-plugin_activate-success' );
			}
