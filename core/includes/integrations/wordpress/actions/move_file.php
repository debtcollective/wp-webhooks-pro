		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $source_path ) && ! empty( $destination_path ) ){
				...
				if( is_array( $check ) && $check['success'] == true ){
					$return_args['msg'] = WPWHPRO()->helpers->translate("The file was successfully moved.", 'action-move_file-success' );
					$return_args['success'] = true;
				} else {
					$return_args['msg'] = WPWHPRO()->helpers->translate("File was not moved because of an error.", 'action-move_file-failure' );
				}
			}
