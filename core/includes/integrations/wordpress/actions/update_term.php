...
<ol>
	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $term_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please define the term_id argument first.", 'action-update_term' );
				return $return_args;
			}

			if( empty( $taxonomy ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please define the taxonomy argument first.", 'action-update_term' );
				return $return_args;
			}
			...
			if( is_wp_error( $term ) ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while fetching your term.", 'action-update_term' );
				return $return_args;
			}

			if( empty( $term ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "We could not find any term for your given term_id", 'action-update_term' );
				return $return_args;
			}
			...
			if ( ! is_wp_error( $term_data ) ) {
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The taxonomy term was successfully updated.", 'action-update_term' );
			} else {
				$return_args['data'] = $term_data;
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Error while updating the taxonomy term.", 'action-update_term' );
			}
