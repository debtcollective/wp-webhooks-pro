...
<ol>
	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $file ) ){
				$check = $file_helpers->delete_file( $file );
				if( $check ){
					$return_args['msg'] = WPWHPRO()->helpers->translate("File successfully deleted.", 'action-delete-file-success' );
					$return_args['success'] = true;
				} else {
					$return_args['msg'] = WPWHPRO()->helpers->translate("File was not deleted because of an error or because it does not exist.", 'action-delete-file-failure' );
				}
			}

			if( ! empty( $do_action ) ){
				do_action( $do_action, $return_args, $file );
			}
