<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_fluent_crm_Actions_fcrm_contact_to_list' ) ) :

	/**
	 * Load the fcrm_contact_to_list action
	 *
	 * @since 4.3.1
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_fluent_crm_Actions_fcrm_contact_to_list {

	public function get_details(){

		$translation_ident = "action-fcrm_contact_to_list-content";

			$parameter = array(
				'email'			=> array( 'short_description' => WPWHPRO()->helpers->translate( 'Set the email of the contact/user you want to add the lists to.', $translation_ident ) ),
				...
				'lists'	=> array( 'required' => true, 'short_description' => WPWHPRO()->helpers->translate( 'Add the lists you want to add the user to. This argument accepts a comma-separated string, as well as a JSON construct.', $translation_ident ) ),
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "In case you want to add multiple lists to the user, you can either comma-separate them like <code>2,3,12,44</code>, or you can add them via a JSON construct:", $translation_ident ); ?>
<pre>{
  23,
  3,
  44
		...
		$parameter['lists']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>fcrm_contact_to_list</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo WPWHPRO()->helpers->translate( "The email of the contact we add the lists to.", $translation_ident ); ?>
	</li>
	<li>
	...
	<li>
		...
		<?php echo WPWHPRO()->helpers->translate( "An array of the lists that have been added to the user.", $translation_ident ); ?>
	</li>
</ol>
		<?php
		...
		$returns_code = array (
            'success' => true,
            'msg' => 'Lists have been added to the given contact.',
            'data' => 
            array (
              'contact' => 
              array (
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
			'webhook_name' => 'Add contact to lists',
			'webhook_slug' => 'fcrm_contact_to_list',
			'steps' => array(
				...
				WPWHPRO()->helpers->translate( 'Please also set the <strong>lists</strong> argument. This argument accepts a comma-separated list of list ids, as well as a JSON with each id on a separate line. Please see the argument definition for further information.', $translation_ident ),
			),
		) );

		return array(
			'action'			=> 'fcrm_contact_to_list', //required
			'name'			   => WPWHPRO()->helpers->translate( 'Add contact to list', $translation_ident ),
			'sentence'			   => WPWHPRO()->helpers->translate( 'add a contact to a list', $translation_ident ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => WPWHPRO()->helpers->translate( 'Add a contact to one or multiple lists within FluentCRM.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $email ) && empty( $user_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set either the user_email or the user_id argument.", 'action-fcrm_contact_to_list-error' );
				return $return_args;
			}

			if( empty( $lists ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the lists argument.", 'action-fcrm_contact_to_list-error' );
				return $return_args;
			}
            ...
            if( empty( $validated_user_email ) ){
                $return_args['msg'] = WPWHPRO()->helpers->translate( "We could not find a contact for your given email or id.", 'action-fcrm_contact_to_list-error' );
				return $return_args;
            }

            $contact = $fcrm_helpers->get_contact( 'email', $validated_user_email );
			if( empty( $contact ) ) {
                $return_args['msg'] = WPWHPRO()->helpers->translate( "We could not fetch the lists from your given contact.", 'action-fcrm_contact_to_list-error' );
				return $return_args;
            }

			
			if( is_array( $validated_lists ) ){
				$contact->attachLists( $validated_lists );

				$return_args['success'] = true;
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Lists have been added to the given contact.", 'action-fcrm_contact_to_list-success' );
				$return_args['data']['contact'] = $contact;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Error: There was an issue validating the lists.", 'action-fcrm_contact_to_list-error' );
			}
