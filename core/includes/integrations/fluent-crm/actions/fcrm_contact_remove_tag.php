...
if ( ! class_exists( 'WP_Webhooks_Integrations_fluent_crm_Actions_fcrm_contact_remove_tag' ) ) :
	...
	public function get_details(){
		...
		$translation_ident = "action-fcrm_contact_remove_tag-content";

			$parameter = array(
				'email'			=> array( 'short_description' => WPWHPRO()->helpers->translate( 'Set the email of the contact/user you want to remove the tags from.', $translation_ident ) ),
				...
				'tags'	=> array( 'required' => true, 'short_description' => WPWHPRO()->helpers->translate( 'Add the tags you want to remove from the user. This argument accepts a comma-separated string, as well as a JSON construct.', $translation_ident ) ),
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "In case you want to remove multiple tags from the contact, you can either comma-separate them like <code>2,3,12,44</code>, or you can remove them via a JSON construct:", $translation_ident ); ?>
<pre>{
  23,
  3,
  44
		...
		$parameter['tags']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>fcrm_contact_remove_tag</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo WPWHPRO()->helpers->translate( "The email of the contact we removed the tags from.", $translation_ident ); ?>
	</li>
	<li>
	...
	<li>
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
			'webhook_name' => 'Remove tags from contact',
			'webhook_slug' => 'fcrm_contact_remove_tag',
			'steps' => array(
		...
		return array(
			...
			'name'			   => WPWHPRO()->helpers->translate( 'Remove tags from contact', $translation_ident ),
			'sentence'			   => WPWHPRO()->helpers->translate( 'remove one or multiple tags from a contact', $translation_ident ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => WPWHPRO()->helpers->translate( 'Remove one or multiple tags from a contact within FluentCRM.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $email ) && empty( $user_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set either the user_email or the user_id argument.", 'action-fcrm_contact_remove_tag-error' );
				return $return_args;
			}

			if( empty( $tags ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the tags argument.", 'action-fcrm_contact_remove_tag-error' );
				return $return_args;
			}
            ...
            if( empty( $validated_user_email ) ){
                $return_args['msg'] = WPWHPRO()->helpers->translate( "We could not find a contact for your given email or id.", 'action-fcrm_contact_remove_tag-error' );
				return $return_args;
            }

            $contact = $fcrm_helpers->get_contact( 'email', $validated_user_email );
			if( empty( $contact ) ) {
                $return_args['msg'] = WPWHPRO()->helpers->translate( "We could not fetch the current tags from your given contact.", 'action-fcrm_contact_remove_tag-error' );
				return $return_args;
            }
			...
			if( is_array( $validated_tags ) ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Tags have been removed from the given contact.", 'action-fcrm_contact_remove_tag-success' );
				$return_args['data']['contact'] = $contact;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Error: There was an issue validating the tags.", 'action-fcrm_contact_remove_tag-error' );
			}
