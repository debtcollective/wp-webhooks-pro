...
if ( ! class_exists( 'WP_Webhooks_Integrations_fluent_crm_Triggers_fcrm_contact_removed_from_list' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_fluent_crm_Triggers_fcrm_contact_removed_from_list {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'fluentcrm_contact_removed_from_lists_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	 ...
	 */
	public function fluentcrm_contact_removed_from_lists_callback( $detached_list_ids, $contact ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'fcrm_contact_removed_from_list' );
		$fcrm_helpers = WPWHPRO()->integrations->get_helper( 'fluent-crm', 'fcrm_helpers' );
		$list_ids = $fcrm_helpers->validate_list_ids( $detached_list_ids );
		...
		do_action( 'wpwhpro/webhooks/trigger_fcrm_contact_removed_from_list', $payload, $response_data_array );
	}
