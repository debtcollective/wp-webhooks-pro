...
if ( ! class_exists( 'WP_Webhooks_Integrations_bbpress_Triggers_bbp_topic_favored' ) ) :

 /**
  * Load the bbp_topic_favored trigger
  *
  * @since 5.1
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_bbpress_Triggers_bbp_topic_favored {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'bbp_topic_favored_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "action-bbp_topic_favored-description";

		$parameter = array(
			'user_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The ID of the user who favored the topic.', $translation_ident ) ),
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Topic favored',
			'webhook_slug' => 'bbp_topic_favored',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'bbp_add_user_favorite',
		...
		$settings = array(
			...
			'data' => array(
				'wpwhpro_bbp_topic_favored_trigger_on_forums' => array(
					'id'	 => 'wpwhpro_bbp_topic_favored_trigger_on_forums',
					'type'	=> 'select',
					'multiple'  => true,
					'choices'   => array(),
					'query'			=> array(
		...
		return array(
			'trigger'		   => 'bbp_topic_favored',
			'name'			  => WPWHPRO()->helpers->translate( 'Topic favored', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a topic was favored', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a topic was favored within bbPress.', $translation_ident ),
	...
	public function bbp_topic_favored_callback( $user_id, $topic_id ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'bbp_topic_favored' );
		...
		foreach( $webhooks as $webhook ){
			...
			if( isset( $webhook['settings'] ) ){
				foreach( $webhook['settings'] as $settings_name => $settings_data ){
					...
					if( $settings_name === 'wpwhpro_bbp_topic_favored_trigger_on_forums' && ! empty( $settings_data ) ){
		...
		do_action( 'wpwhpro/webhooks/trigger_bbp_topic_favored', $payload, $response_data_array );
	}
