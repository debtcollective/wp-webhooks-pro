<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_klicktipp_Actions_klickt_untag_subscriber' ) ) :

	/**
	 * Load the klickt_untag_subscriber action
	 *
	 * @since 5.2
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_klicktipp_Actions_klickt_untag_subscriber {

	public function get_details(){

		$translation_ident = "action-klickt_untag_subscriber-content";
		...
		$parameter = array(
			...
			'email_address'		=> array( 
				...
				'short_description' => WPWHPRO()->helpers->translate( 'The email address of the subscriber you want to remove the tag from.', $translation_ident ),
			),
			'tag_id' => array( 
				'required' => true, 
		...
		$returns_code = array (
			'success' => true,
			'msg' => 'The subscriber was successfully untagged.',
		);

		$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
			'webhook_name' => 'Untag subscriber',
			'webhook_slug' => 'klickt_untag_subscriber',
			'steps' => array(
				...
				WPWHPRO()->helpers->translate( 'It is required to set the <strong>email_address</strong> argument. Please set it to the email address of the subscriber you want to untag.', $translation_ident ),
				WPWHPRO()->helpers->translate( 'It is required to set the <strong>tag_id</strong> argument. Please set it to the ID of the tag you want to remove from the subscriber.', $translation_ident ),
			),
		) );

		return array(
			'action'			=> 'klickt_untag_subscriber', //required
			'name'			   => WPWHPRO()->helpers->translate( 'Untag subscriber', $translation_ident ),
			'sentence'			   => WPWHPRO()->helpers->translate( 'untag a subscriber', $translation_ident ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => WPWHPRO()->helpers->translate( 'Untag a subscriber within "KlickTipp".', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $email_address ) || ! is_email( $email_address ) ){
                $return_args['msg'] = WPWHPRO()->helpers->translate( "Please define a valid email address within the email_address argument.", 'action-klickt_untag_subscriber-error' );
				return $return_args;
            }
			...
			if( ! empty( $connector ) ){
				...
				$subscriber = $connector->untag( $email_address, $tag_id_validated );

				if( ! empty( $subscriber ) ){
					$return_args['success'] = true;
					$return_args['msg'] = WPWHPRO()->helpers->translate( "The subscriber was successfully untagged.", 'action-klickt_untag_subscriber-success' );
					$return_args['data']['subscriber'] = $subscriber;
				} else {
					$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while untagging the subscriber.", 'action-klickt_untag_subscriber-success' );
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while loading the KlickTipp helper.", 'action-klickt_untag_subscriber-success' );
			}
