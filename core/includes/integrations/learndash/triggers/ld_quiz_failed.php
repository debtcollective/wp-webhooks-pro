...
if ( ! class_exists( 'WP_Webhooks_Integrations_learndash_Triggers_ld_quiz_failed' ) ) :

 /**
  * Load the ld_quiz_failed trigger
  *
  * @since 4.3.7
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_learndash_Triggers_ld_quiz_failed {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'learndash_quiz_submitted_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "action-ld_quiz_failed-description";

		$parameter = array(
			'user_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The id of the user that failed the course.', $translation_ident ) ),
			'course_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The ID of the failed course.', $translation_ident ) ),
			...
			'question_show_count' => array( 'short_description' => WPWHPRO()->helpers->translate( '(learndash_quiz_submitted) A count of how often the question has been shown.', $translation_ident ) ),
			...
			'time' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) A timestamp of the time the quiz was failed.', $translation_ident ) ),
			...
			'completed' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) A timestamp of the time the quiz was initially completed.', $translation_ident ) ),
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Quiz failed',
			'webhook_slug' => 'ld_quiz_failed',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'learndash_quiz_submitted',
					'url' => 'https://developers.learndash.com/hook/learndash_quiz_submitted/',
				),
			),
			'tipps' => array(
		...
		return array(
			'trigger'		   => 'ld_quiz_failed',
			'name'			  => WPWHPRO()->helpers->translate( 'Quiz failed', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a quiz was failed', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a quiz was failed within LearnDash.', $translation_ident ),
	...
	public function learndash_quiz_submitted_callback( $data, $user ){

		if( 0 !== intval( $data['pass'] ) ){
			return;
		}

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'ld_quiz_failed' );
		...
		do_action( 'wpwhpro/webhooks/trigger_ld_quiz_failed', $payload, $response_data_array );
	}
