	...
	<li>
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user_id ) || empty( $course_ids ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the user_id and course_ids arguments.", 'action-ld_remove_course_access-error' );
				return $return_args;
			}
			...
			if( ! empty( $removed_access ) ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The course access has been successfully removed.", 'action-ld_remove_course_access-success' );
				$return_args['data']['user_id'] = $user_id;
				$return_args['data']['course_ids'] = $course_ids;
				$return_args['data']['removed_access'] = $removed_access;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "No course access has been removed for the given user within Learndash.", 'action-ld_remove_course_access-success' );
				$return_args['data']['user_id'] = $user_id;
				$return_args['data']['course_ids'] = $course_ids;
				$return_args['data']['removed_access'] = $removed_access;
			}
