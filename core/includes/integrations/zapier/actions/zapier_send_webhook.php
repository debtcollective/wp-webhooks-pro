<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_zapier_Actions_zapier_send_webhook' ) ) :

	/**
	 * Load the zapier_send_webhook action
	 *
	 * @since 5.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_zapier_Actions_zapier_send_webhook {

		public function get_details(){

			$translation_ident = "action-zapier_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'Zapier "Catch Hook" URL', $translation_ident ), 
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhooks by Zapier" app from within Zapier via the "Catch Hook" trigger.', $translation_ident ), 
				),
				'method'	=> array( 
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>zapier_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$returns_code = array (
				...
				array (
				  ...
				  array (
					...
					'date' => 'Thu, 24 Mar 2022 09:35:38 GMT',
					'p3p' => 'CP="IDC DSP COR ADM DEVi TAIi PSA PSD IVAi IVDi CONi HIS OUR IND CNT"',
					'response-id' => '623c3b6a-xxxx-xxxx-xxxx-f04be37exxxx',
					'strict-transport-security' => 'max-age=31536000; includeSubDomains; preload',
					'vary' => 'X-Session-Key, Origin',
					'x-frame-options' => 'SAMEORIGIN',
					'x-rate-limit-limit' => '10000',
					'x-rate-limit-remaining' => '9999',
					'x-rate-limit-reset' => '1648114800',
					'x-session-key' => 'anonymous',
					'x-zapier-fast-return' => 'true',
					'x-zapier-revision' => '1fe9088eada37efeed0a1ffxxxxxxx90e360',
					'x-cache' => 'Miss from cloudfront',
					'via' => '1.1 e7067e3a166cb7dxxxxxxxxxxx.cloudfront.net (CloudFront)',
					'x-amz-cf-pop' => 'DXB50-C1',
					'x-amz-cf-id' => '0k7NxxxxxxxxxxxxxxxxxxxxxxxxxxxxI_DhmZBgEQiHDLZQ==',
				  ),
				  'cookies' => 
				  array (
				  ),
				  ...
				  'content' => 
				  array (
					'id' => '623c3b6a-xxxx-xxxx-xxxx-f04be37exxxx',
					'request_id' => '623c3b6a-xxxx-xxxx-xxxx-f04be37exxxx',
					'attempt' => '623c3b6a-xxxx-xxxx-xxxx-f04be37exxxx',
					'status' => 'success',
				  ),
				  'response' => 
				  array (
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to Zapier webhook app',
				'webhook_slug' => 'zapier_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhooks by Zapier" app of Zapier via the "Catch Hook" trigger.', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'zapier_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to Zapier webhook app', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the Zapier webhook app', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to the "Webhooks by Zapier" app of Zapier from your WordPress website, via the "Catch hook" trigger.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-zapier_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-zapier_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-zapier_send_webhook-success' );
			}
