...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_subscriptions_Triggers_wcs_subscription_cancelled' ) ) :

 /**
  * Load the wcs_subscription_cancelled trigger
  *
  * @since 5.2
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_woocommerce_subscriptions_Triggers_wcs_subscription_cancelled {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'wcs_subscription_cancelled_callback' ),
				'priority' => 20,
				'arguments' => 1,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "trigger-wcs_subscription_cancelled-description";
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Subscription cancelled',
			'webhook_slug' => 'wcs_subscription_cancelled',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'woocommerce_subscription_status_cancelled',
		...
		return array(
			'trigger'		   => 'wcs_subscription_cancelled',
			'name'			  => WPWHPRO()->helpers->translate( 'Subscription cancelled', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a subscription was cancelled', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a subscription was cancelled within WooCommerce Subscriptions.', $translation_ident ),
	...
	public function wcs_subscription_cancelled_callback( $subscription ){	

		$subscription_id = $subscription->get_id();
		$user_id = $subscription->get_user_id();
		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'wcs_subscription_cancelled' );
		...
		do_action( 'wpwhpro/webhooks/trigger_wcs_subscription_cancelled', $payload, $response_data_array );
	}
