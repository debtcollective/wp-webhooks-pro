<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_pabbly_connect_Actions_pabbly_send_webhook' ) ) :

	/**
	 * Load the pabbly_send_webhook action
	 *
	 * @since 5.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_pabbly_connect_Actions_pabbly_send_webhook {

		public function get_details(){

			$translation_ident = "action-pabbly_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'Pabbly Connect webhook URL', $translation_ident ), 
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhook by Pabbly" app from within Pabbly Connect.', $translation_ident ), 
				),
				'method'	=> array( 
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>pabbly_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$returns_code = array (
				...
				array (
				  ...
				  'headers' => 
				  array (
					'date' => 'Thu, 24 Mar 2022 11:50:52 GMT',
					'content-type' => 'text/html; charset=UTF-8',
					'set-cookie' => 'ci_connect_pabbly_sesion=lnvxxxxxxxxxxxxxxxxxxxxxxxxx; expires=Thu, 24-Mar-2022 13:50:51 GMT; Max-Age=7200; path=/; HttpOnly',
					...
					'report-to' => '{"endpoints":[{"url":"https:\\/\\/a.nel.cloudflare.com\\/report\\/v3?s=xxxxxxx"}],"group":"cf-nel","max_age":604800}',
					'nel' => '{"success_fraction":0,"report_to":"cf-nel","max_age":604800}',
					'server' => 'cloudflare',
					'cf-ray' => '6f0f3xxxxxxxx',
					'content-encoding' => 'br',
				  ),
				  'cookies' => 
				  array (
					0 => 
					array (
					  'name' => 'ci_connect_pabbly_sesion',
					  'value' => 'xxxxxxxxxxx',
					  'expires' => 1648129851,
					  'path' => '/',
					  'domain' => 'connect.pabbly.com',
					  'port' => NULL,
					  'host_only' => true,
					),
				  ),
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to Pabbly Connect webhook app',
				'webhook_slug' => 'pabbly_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhooks by Pabbly Connect" app of Pabbly Connect via the "Catch Hook" trigger.', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'pabbly_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to Pabbly Connect webhook app', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the Pabbly Connect webhook app', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to the "Webhook by Pabbly" app of Pabbly Connect from your WordPress website.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-pabbly_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-pabbly_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-pabbly_send_webhook-success' );
			}
