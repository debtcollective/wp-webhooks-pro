...
if ( ! class_exists( 'WP_Webhooks_Integrations_restrict_content_pro_Triggers_rcp_membership_activated_paid' ) ) :

 /**
  * Load the rcp_membership_activated_paid trigger
  *
  * @since 4.3.6
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_restrict_content_pro_Triggers_rcp_membership_activated_paid {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'rcp_membership_activated_paid_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "action-rcp_membership_activated_paid-description";
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			...
			'webhook_slug' => 'rcp_membership_activated_paid',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'rcp_membership_post_activate',
		...
		return array(
			'trigger'		   => 'rcp_membership_activated_paid',
			'name'			  => WPWHPRO()->helpers->translate( 'Paid membership activated', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a paid membership was activated', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a paid membership was activated within Restrict Content Pro.', $translation_ident ),
	...
	public function rcp_membership_activated_paid_callback( $membership_id, $membership ){

		if( empty( $membership ) || ! $membership->is_paid() ){
            return;
        }

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'rcp_membership_activated_paid' );
		...
		do_action( 'wpwhpro/webhooks/trigger_rcp_membership_activated_paid', $payload, $response_data_array );
	}
	...
	public function get_demo( $options = array() ) {
		...
		$data = array (
			...
			array (
			  ...
			  'gateway_subscription_id' => 'demo-subscription-id',
			  'subscription_key' => '',
			  'get_upgraded_from' => '0',
			  'was_upgrade' => false,
			  'payment_plan_completed_date' => NULL,
			  'notes' => 'February 21, 2022 11:11:26 - Membership activated.',
