  ...
  */
  class WP_Webhooks_Integrations_advanced_custom_fields_Triggers_acf_term_field_updated {
	...
	public function get_details(){
		...
		$translation_ident = "action-acf_term_field_updated-description";

		$parameter = array(
			'meta_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The unique ID of the meta value.', $translation_ident ) ),
			'term_id' => array( 'short_description' => WPWHPRO()->helpers->translate( '(Integer) The ID of the term this meta value was updated on.', $translation_ident ) ),
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'ACF term field updated',
			'webhook_slug' => 'acf_term_field_updated',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'update_term_meta',
			...
			'tipps' => array(
				WPWHPRO()->helpers->translate( 'It is also posible to fire trigger on specific term IDs. To do that, simply specify the term IDs within the webhook URL settings.', $translation_ident ),
		...
		$settings = array(
			...
			'data' => array(
				'wpwhpro_advanced_custom_fields_trigger_on_selected_term_ids' => array(
					...
					'type'		=> 'text',
					'label'	   => WPWHPRO()->helpers->translate( 'Trigger on selected term IDs', $translation_ident ),
					'placeholder' => '',
					'required'	=> false,
					'description' => WPWHPRO()->helpers->translate( 'Add only the term IDs you want to fire the trigger on. You can also choose multiple ones by comma-separating them. If none are added, all are triggered.', $translation_ident )
				),
		...
		return array(
			...
			'name'			  => WPWHPRO()->helpers->translate( 'ACF term field updated', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'an ACF term field was updated', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a term field was updated within Advanced Custom Fields.', $translation_ident ),
