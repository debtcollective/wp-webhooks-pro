...
if ( ! class_exists( 'WP_Webhooks_Integrations_advanced_custom_fields_Actions_acf_manage_term_meta_data' ) ) :
	...
	public function get_details(){
		...
		$translation_ident = "action-acf_manage_term_meta_data-description";

		$parameter = array(
			'term_id' => array( 'required' => true, 'short_description' => WPWHPRO()->helpers->translate( 'The ID of the term you want to perform the action for.', $translation_ident ) ),
			'meta_update' => array( 
				'type' => 'repeater',
...
<pre>
{
	"meta_key": "Meta Value"
		...
		$parameter['manage_acf_data']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The do_action argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the acf_manage_term_meta_data action was fired.", $translation_ident ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Manage ACF term meta',
				'webhook_slug' => 'acf_manage_term_meta_data',
				'steps' => array(
					WPWHPRO()->helpers->translate( "Please also set the <strong>term_id</strong> argument. Set it to the ID of your chosen term.", $translation_ident ),
				...
				'tipps' => array(
					WPWHPRO()->helpers->translate( "To create term meta values visually, you can use our meta value generator at: <a title=\"Visit our meta value generator\" href=\"https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/\" target=\"_blank\">https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/</a>.", $translation_ident ),
				)
			) );

			return array(
				'action'			=> 'acf_manage_term_meta_data',
				'name'			  => WPWHPRO()->helpers->translate( 'Manage ACF term meta', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'add, update, or delete ACF term meta', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'Add, update, or delete custom term meta data within "Advanced Custom Fields".', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			$translation_ident = "action-acf_manage_term_meta_data-execute";
			$return_args = array(
				'success' => false,
				'msg' => '',
				'data' => array(),
			...
			if( empty( $term_id ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the term_id argument first.", $translation_ident );
				return $return_args;
			}

			$term = get_term( $term_id );
			if( empty( $term ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The term you try to update does not exist.", $translation_ident );
				return $return_args;
			}
