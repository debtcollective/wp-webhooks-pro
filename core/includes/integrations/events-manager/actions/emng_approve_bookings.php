<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_events_manager_Actions_emng_approve_bookings' ) ) :

	/**
	 * Load the emng_approve_bookings action
	 *
	 * @since 4.3.6
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_events_manager_Actions_emng_approve_bookings {

	public function get_details(){

		$translation_ident = "action-emng_approve_bookings-content";
...
<pre>all</pre>
		...
		$parameter['event_ids']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>emng_approve_bookings</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	<li>
		...
		$returns_code = array (
			'success' => true,
			'msg' => 'The bookings have been approved.',
			'data' => 
			array (
			  'user_id' => 148,
			  'events' => 
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
			...
			'webhook_slug' => 'emng_approve_bookings',
			'steps' => array(
		...
		return array(
			'action'			=> 'emng_approve_bookings', //required
			'name'			   => WPWHPRO()->helpers->translate( 'Approve bookings', $translation_ident ),
			'sentence'			   => WPWHPRO()->helpers->translate( 'approve one or multiple bookings', $translation_ident ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => WPWHPRO()->helpers->translate( 'Approve one, multiple, or all bookings for a user within Events Manager.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the user argument to either the user id or user email.", 'action-emng_approve_bookings-error' );
				return $return_args;
			}

			if( empty( $event_ids ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the event_ids argument.", 'action-emng_approve_bookings-error' );
				return $return_args;
			}
            ...
            if( empty( $user_id ) ){
                $return_args['msg'] = WPWHPRO()->helpers->translate( "We could not find a user for your given user data.", 'action-emng_approve_bookings-error' );
				return $return_args;
            }
				...
				if( count( $em_bookings->bookings ) > 0 ){
					...
					foreach( $em_bookings as $em_booking ){
						if( $trigger_all_events || in_array( $em_booking->event_id, $validated_events ) ){
							...
							$em_booking->set_status( 1, $send_email, $ignore_spaces );
						}
					}

					$return_args['success'] = true;
					$return_args['msg'] = WPWHPRO()->helpers->translate( "The bookings have been approved.", 'action-emng_approve_bookings-success' );
					...
					$return_args['msg'] = WPWHPRO()->helpers->translate( "The given user has no bookings.", 'action-emng_approve_bookings-success' );
				}
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "We could not find a booking person for the given user.", 'action-emng_approve_bookings-success' );
			}
