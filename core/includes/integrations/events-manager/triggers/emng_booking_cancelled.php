...
if ( ! class_exists( 'WP_Webhooks_Integrations_events_manager_Triggers_emng_booking_cancelled' ) ) :

 /**
  * Load the emng_booking_cancelled trigger
  *
  * @since 4.3.6
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_events_manager_Triggers_emng_booking_cancelled {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'emng_booking_cancelled_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "action-emng_booking_cancelled-description";
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Booking cancelled',
			'webhook_slug' => 'emng_booking_cancelled',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'em_booking_set_status',
		...
		return array(
			'trigger'		   => 'emng_booking_cancelled',
			'name'			  => WPWHPRO()->helpers->translate( 'Booking cancelled', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a booking was cancelled', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a booking was cancelled within Events Manager.', $translation_ident ),
	...
	public function emng_booking_cancelled_callback( $result, $booking ){

		if( empty( $booking ) || $booking->booking_status !== 3 || $booking->previous_status === $booking->booking_status ){
			return;
		}

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'emng_booking_cancelled' );
		...
		do_action( 'wpwhpro/webhooks/trigger_emng_booking_cancelled', $payload, $response_data_array );
	}
	...
	public function get_demo( $options = array() ) {
		...
		$data = array (
			...
			array (
			  ...
			  'feedback_message' => 'Booking cancelled.',
			  'errors' => 
			  array (
			  ),
			  'mails_sent' => 0,
