...
if ( ! class_exists( 'WP_Webhooks_Integrations_givewp_Triggers_give_donation_cancelled' ) ) :
  ...
  */
  class WP_Webhooks_Integrations_givewp_Triggers_give_donation_cancelled {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'give_donation_cancelled_callback' ),
				'priority' => 20,
				'arguments' => 3,
				'delayed' => true,
			),
	...
	public function give_donation_cancelled_callback( $payment_id, $status, $old_status ){

		if( $status !== 'cancelled' ){
			return;
		}

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'give_donation_cancelled' );
		...
		do_action( 'wpwhpro/webhooks/trigger_give_donation_cancelled', $payload, $response_data_array );
	}
