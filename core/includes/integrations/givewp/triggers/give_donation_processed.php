...
if ( ! class_exists( 'WP_Webhooks_Integrations_givewp_Triggers_give_donation_processed' ) ) :

 /**
  * Load the give_donation_processed trigger
  *
  * @since 4.3.4
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_givewp_Triggers_give_donation_processed {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				...
				'callback' => array( $this, 'give_donation_processed_callback' ),
				'priority' => 20,
				'arguments' => 3,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$translation_ident = "action-give_donation_processed-description";
		...
		$description = WPWHPRO()->webhook->get_endpoint_description( 'trigger', array(
			'webhook_name' => 'Donation processed',
			'webhook_slug' => 'give_donation_processed',
			'post_delay' => true,
			'trigger_hooks' => array(
				array( 
					'hook' => 'give_update_payment_status',
		...
		return array(
			'trigger'		   => 'give_donation_processed',
			'name'			  => WPWHPRO()->helpers->translate( 'Donation processed', $translation_ident ),
			'sentence'			  => WPWHPRO()->helpers->translate( 'a donation is processed', $translation_ident ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => WPWHPRO()->helpers->translate( 'This webhook fires as soon as a donation is processed within GiveWP.', $translation_ident ),
	...
	public function give_donation_processed_callback( $payment_id, $status, $old_status ){

		if( $status !== 'processing' ){
			return;
		}

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'give_donation_processed' );
		...
		do_action( 'wpwhpro/webhooks/trigger_give_donation_processed', $payload, $response_data_array );
	}
