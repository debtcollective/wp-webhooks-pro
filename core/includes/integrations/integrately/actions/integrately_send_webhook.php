...
if ( ! class_exists( 'WP_Webhooks_Integrations_integrately_Actions_integrately_send_webhook' ) ) :
	...
	class WP_Webhooks_Integrations_integrately_Actions_integrately_send_webhook {
		...
		public function get_details(){
			...
			$translation_ident = "action-integrately_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'Integrately "Webhook" URL', $translation_ident ), 
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhook / API" integration from within Integrately.', $translation_ident ), 
				),
				'method'	=> array( 
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>integrately_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to Integrately webhook app',
				'webhook_slug' => 'integrately_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhook / API" integration of Integrately.', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'integrately_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to Integrately webhook', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the Integrately webhook integration', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to the "Webhook / API" integration of Integrately from your WordPress website.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-integrately_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-integrately_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-integrately_send_webhook-success' );
			}
