<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_automate_io_Actions_automateio_send_webhook' ) ) :

	/**
	 * Load the automateio_send_webhook action
	 *
	 * @since 5.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_automate_io_Actions_automateio_send_webhook {

		public function get_details(){

			$translation_ident = "action-automateio_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'Automate.io webhook URL', $translation_ident ), 
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhooks" app from within Automate.io via the "Incoming Hook" trigger.', $translation_ident ), 
				),
				'method'	=> array( 
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>automateio_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
...
<ol>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$returns_code = array (
				...
				array (
				  ...
				  'headers' => 
				  array (
					'date' => 'Thu, 24 Mar 2022 18:01:37 GMT',
					'content-type' => 'application/json; charset=utf-8',
					'content-length' => '20',
					'server' => 'nginx',
					'x-powered-by' => 'Express',
					'access-control-allow-origin' => '*',
					'etag' => 'x/xxxxxxxxxxx/xxxxxxxxxx/xxxxxxxxxxxx"',
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to Automate.io webhook',
				'webhook_slug' => 'automateio_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhooks" app of Automate.io via the "Incoming Hook" trigger.', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'automateio_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to Automate.io webhook', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the Automate.io webhook app', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to the "Webhook" app of Automate.io via the "Incoming Hook" trigger from your WordPress website.', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-automateio_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-automateio_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-automateio_send_webhook-success' );
			}
