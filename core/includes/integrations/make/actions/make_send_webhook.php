<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_make_Actions_make_send_webhook' ) ) :

	/**
	 * Load the make_send_webhook action
	 *
	 * @since 5.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_make_Actions_make_send_webhook {

		public function get_details(){

			$translation_ident = "action-make_send_webhook-description";
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => WPWHPRO()->helpers->translate( 'Make (Integromat) webhooks URL', $translation_ident ),
					'short_description' => WPWHPRO()->helpers->translate( '(string) The URL you want to send the data to from the "Webhooks" app from within Make (Integromat)', $translation_ident ),
				),
				'method'	=> array( 
					'default_value' => 'POST', 
					'type' => 'select', 
				...
				'limit_response_size'	=> array( 'short_description' => WPWHPRO()->helpers->translate( '(integer) Limit the response size of the data coming back from the recpient. Default: null', $translation_ident ) ),
				'cookies'	   => array( 'type' => 'repeater', 'multiple' => true, 'short_description' => WPWHPRO()->helpers->translate( '(string) A JSON formatted string containing additional cookie data.', $translation_ident ) ),
				'do_action'	=> array( 'short_description' => WPWHPRO()->helpers->translate( 'Advanced: Register a custom action after the webhook fires.', $translation_ident ) ),
			);
...
<?php echo WPWHPRO()->helpers->translate( "Set this argument to no to use unverified SSL connections for this URL.", $translation_ident ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo WPWHPRO()->helpers->translate( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>make_send_webhook</strong> action was fired.", $translation_ident ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$returns_code = array (
				...
				array (
				  ...
				  array (
					...
					'x-request-id' => 'd26ddbd7-0000-41d6-0000-79cc682490fd',
					'x-token-id' => '15c5d3d7-0000-4504-0000-a98d43fe5d3d',
					'cache-control' => 'no-cache, private',
					'date' => 'Wed, 23 Mar 2022 06:42:35 GMT',
			...
			$description = WPWHPRO()->webhook->get_endpoint_description( 'action', array(
				'webhook_name' => 'Send data to webhook app',
				'webhook_slug' => 'make_send_webhook',
				'steps' => array(
					WPWHPRO()->helpers->translate( 'It is also required to set the <strong>url</strong> argument. Please set it to the recipient URL from within the "Webhooks" app of Make (Integromat).', $translation_ident ),
				),
			) );

			return array(
				'action'			=> 'make_send_webhook',
				'name'			  => WPWHPRO()->helpers->translate( 'Send data to webhook app', $translation_ident ),
				'sentence'			  => WPWHPRO()->helpers->translate( 'send data to the Make (Integromat) webhook app', $translation_ident ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => WPWHPRO()->helpers->translate( 'This webhook action allows you to send data to a webhook request from your WordPress website to the "Webhooks" app of Make (Integromat).', $translation_ident ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = WPWHPRO()->helpers->translate( "Please set the url argument.", 'action-make_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = WPWHPRO()->helpers->translate( "The webhook request was sent successfully.", 'action-make_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = WPWHPRO()->helpers->translate( "An error occured while sending the webhook request.", 'action-make_send_webhook-success' );
			}
